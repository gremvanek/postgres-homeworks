...
-- Удаление foreign key(с условием если он существует)
ALTER TABLE order_details
DROP CONSTRAINT IF EXISTS FK_order_details_products;

-- Удалить товары из order_details, которые связаны с отсутствующими product_id в products
DELETE FROM order_details
WHERE product_id NOT IN (SELECT product_id FROM products);

-- Восстановить ограничение foreign key
ALTER TABLE order_details
